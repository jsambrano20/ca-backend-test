...
using BackendTest.Models.TokenAuth;
...
{
    public abstract class BackendTestWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static BackendTestWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(BackendTestWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<BackendTestDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<BackendTestDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<BackendTestDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<BackendTestDbContext, Task<T>> func)
        {
            T result;
